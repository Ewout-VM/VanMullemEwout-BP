...
Daarnaast moet de oplossing zorgen voor een betere opvolging van uitgevoerde taken en een efficiëntere samenwerking tussen verschillende diensten binnen het onboarding- en offboardingsproces. Het uiteindelijke doel is om de administratieve belasting te verminderen, de foutenmarge te verkleinen en de doorlooptijd van beide processen te beperken.

\section{Requirements van de oplossing}

Op basis van de literatuurstudie en de analyse van de huidige onboardingprocedure worden de requirements voor de oplossing bepaald. Hierbij wordt onderscheid gemaakt tussen functionele en niet-functionele requirements. De functionele requirements beschrijven welke functionaliteiten de oplossing moet of bij voorkeur moet aanbieden. De niet-functionele requirements beschrijven de voorwaarden waaraan de oplossing moet voldoen.

\subsection{Functionele requirements}

De functionele requirements worden ingedeeld volgens de MoSCoW-methode. Hierbij wordt onderscheid gemaakt tussen must-have, should-have, could-have en would-have requirements.

\subsubsection{Must-have requirements}

De oplossing moet minimaal voldoen aan de volgende requirements:

\begin{itemize}
    \item De oplossing moet automatisch een gebruikersaccount kunnen aanmaken in Active Directory op basis van personeelsgegevens.
    \item De oplossing moet de relevante gebruikersattributen binnen Active Directory automatisch kunnen instellen.
    \item De oplossing moet Microsoft 365-licenties automatisch kunnen toekennen aan nieuwe gebruikers.
    \item De oplossing moet op basis van de dienst van de medewerker de vereiste basisrechten automatisch kunnen toekennen.
    \item De oplossing moet de status van een uitgevoerde onboarding beschikbaar kunnen maken voor andere betrokken diensten binnen de organisatie.
\end{itemize}

\subsubsection{Should-have requirements}

De oplossing zou bij voorkeur ook de volgende functionaliteiten moeten ondersteunen:

\begin{itemize}
    \item De oplossing zou gebruikers automatisch rechten op gedeelde postvakken moeten kunnen toekennen.
    \item De oplossing zou gebruikers automatisch aan relevante distributielijsten binnen Microsoft 365 moeten kunnen toevoegen.
\end{itemize}

Deze functionaliteiten verminderen bijkomende manuele configuraties binnen de ICT-dienst en verhogen de mate van automatisering van het onboardingproces.

\subsubsection{Could-have requirements}

De volgende functionaliteiten vormen een meerwaarde, maar zijn niet noodzakelijk voor de basiswerking van de oplossing:

\begin{itemize}
    \item De oplossing zou automatisch een welkomstmail naar nieuwe medewerkers kunnen versturen met praktische informatie en relevante handleidingen.
    \item De oplossing zou onderdelen van het offboardingproces kunnen automatiseren.
\end{itemize}

Hoewel offboarding een belangrijk onderdeel vormt van identiteits- en toegangsbeheer, ligt de primaire focus van deze bachelorproef op het onboardingproces. Daarom wordt offboarding als een optionele uitbreiding beschouwd.

\subsubsection{Would-have requirements}

De volgende functionaliteit is wenselijk, maar kan binnen de huidige infrastructuur niet betrouwbaar worden gerealiseerd:

\begin{itemize}
    \item De oplossing zou automatisch alle vereiste rechten op de file server moeten kunnen toekennen.
\end{itemize}

Binnen de huidige omgeving bestaat een complexe rechtenstructuur waarbij bepaalde toegangen historisch rechtstreeks aan individuele gebruikers werden toegewezen in plaats van via veiligheidsgroepen. Hierdoor kan deze functionaliteit momenteel niet op een betrouwbare manier worden geautomatiseerd. Een volledige herstructurering van de bestaande rechtenomgeving zou noodzakelijk zijn om deze requirement te realiseren.

\subsection{Niet-functionele requirements}

Naast de functionele requirements moet de oplossing voldoen aan verschillende niet-functionele requirements:

\begin{itemize}
    \item De oplossing moet compatibel zijn met de bestaande hybride IT-infrastructuur van het gemeentebestuur van Berlare.
    \item De oplossing moet zowel lokale on-premisesystemen als cloudtoepassingen binnen Microsoft 365 kunnen ondersteunen.
    \item De oplossing moet veilig functioneren binnen de bestaande infrastructuur.
    \item Alleen bevoegde medewerkers mogen toegang hebben tot de automatiseringsflows en gevoelige configuraties.
    \item De oplossing moet voldoende flexibel zijn om toekomstige uitbreidingen en aanpassingen mogelijk te maken.
    \item De oplossing moet onderhoudbaar zijn zodat wijzigingen in de bestaande infrastructuur of het onboardingproces kunnen worden verwerkt.
\end{itemize}

\section{Analyse van automatiseerbare processen}

In de volgende sectie volgt de onderverdeling van de taken aan de hand van automatiseerbaarheid.
...
Daarnaast is offboarding vanuit beveiligingsoogpunt een belangrijk proces. Het tijdig verwijderen van toegangsrechten vermindert het risico op ongewenste toegang tot interne systemen en gegevens nadat een medewerker uit dienst is gegaan.
...
Binnen het onderzoek werden verschillende automatiseringsplatformen on\-der\-zocht die gebruikt kunnen worden voor workflowautomatisering en systeemintegratie.
...
Verder ondersteunt Power Automate de combinatie van cloudflows en lokale desk\-top\-flows via Power Automate Desktop en Machine Runtime, waardoor communicatie met lokale systemen mogelijk wordt binnen een hybride omgeving.
...
Daarnaast moet de oplossing niet alleen nieuwe accounts kunnen aanmaken, maar ook bestaande gebruikersaccounts veilig kunnen aanpassen of deactiveren tijdens het offboardingproces. Voor zowel onboarding als ofboarding moet communicatie mogelijk zijn met Exchange Online voor het beheren van mailboxrechten, distributiegroepen en gedeelde postvakken.

\section{Ontwerp van de oplossing}

In de volgende sectie is het ontwerp van de oplossing. 
...
De invoer van gebruikersgegevens gebeurt via Microsoft Forms. Wanneer een nieu\-we aanvraag wordt verzonden, worden de gegevens verwerkt binnen een cloudflow in Power Automate. Deze flow verwerkt de ontvangen gegevens en bereidt deze voor op verdere verwerking binnen de lokale omgeving.
...
Nadien worden de distributielijsten en gedeelde postvakken toegevoegd of verwijderd bij de gebruiker. De configuratie van distributielijsten en gedeelde postvakken gebeurt via PowerShell-scripts die communiceren met Exchange Online. Hiervoor werd gekozen omdat Power Automate momenteel geen volwaardige connectoren aanbiedt voor het beheren van mailboxrechten binnen Exchange Online.

Als laatste stap binnen het configuratieproces wordt automatisch een welkomstmail verzonden naar de nieuwe medewerker. Deze e-mail bevat verschillende handleidingen en praktische informatie die noodzakelijk zijn om vlot van start te kunnen gaan tijdens de eerste werkdag. Hierbij kan gedacht worden aan de policy over het gebruik van het draadloze netwerk, het gebruik van printers en andere interne ICT-toepassingen.
...
De offboarding start vanuit een afzonderlijk Microsoft Forms-formulier waarin de personeelsdienst de gegevens van de medewerker invoert, waaronder het e\-mail\-adres, de einddatum van het contract en de reden van uitdiensttreding.

Omdat gebruikers pas gedeactiveerd mogen worden nadat de laatste werkdag verstreken is, werd een wachtmechanisme voorzien binnen de cloudflow van Power Automate. De flow blijft actief totdat de opgegeven einddatum bereikt wordt.
...
Ook tijdens offboarding blijft deze synchronisatie belangrijk zodat wijzigingen aan gebruikersaccounts en groepslidmaatschappen correct doorstromen naar de cloud\-om\-ge\-ving.
