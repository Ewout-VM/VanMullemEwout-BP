...
Figuur \ref{fig:VoorbeeldOutput} toont voorbeeldoutput die gebruikt werd om te valideren dat de gegevens correct gebruikt werden.
...
\begin{figure}[htbp]
    ...
    \caption[Desktop flow deel 1]{Deze afbeelding bevat de eerste flow op de desktop. In deze flow worden de gebruikers aangemaakt en variabelen terug gestuurd naar de cloud flow.}
...
\begin{figure}[htbp]
    ...
    \caption[Controle cloudgebruiker en toevoeging groepen]{Deze afbeelding bevat de controle of de gebruiker aangemaakt is in Entra ID. Verder bevat deze ook de stappen om groepen toe te voegen.}
    \label{fig:Do_until}
\end{figure}
...
Onderstaand fragment toont de parameters die vanuit de desktopflow doorgestuurd worden naar het script.
...
Wanneer een fout optreedt tijdens het uitvoeren van een opdracht, wordt een waarschuwing of foutmelding weggeschreven zodat deze later geraadpleegd kan worden binnen Power Automate of tijdens troubleshooting.

Onderstaand fragment toont een voorbeeld van foutafhandeling tijdens het toevoegen van een gebruiker aan een gedeeld postvak.
...
In deze handleidingen zijn stappenplannen te vinden met betrekking tot het gebruik van de VPN, mails vrijgeven die tegengehouden worden door de spamfilter en printen. In de mail zelf zijn ook een paar praktische afspraken in verband met het gebruik van wifi. Ook de uitleg over MFA is te vinden in deze mail. Ook is er in de mail een linkje naar de helpdesk waar ze al hun problemen moeten melden en uiteraard ook geholpen worden indien nodig. 

Binnen Power Automate moeten deze bijlagen eerst geladen worden. Dit gebeurd met de stap ``Get file content''. Voor de 4 bijlagen gebeurd dit dus op identieke, parallelle wijze. Eens deze opgeladen zijn in Power Automate, kunnen ze geselecteerd worden in de stap ``Send an email from a shared mailbox''. Hierin staat de mailadres dat de mail verstuurd, als ook de ontvanger. Dit is dan op dynamische wijze ook weer de UPN variabele die initieel via de desktopflow naar de cloud wordt gestuurd. Net zoals bij een normale mail hoort er ook een onderwerp en de gehele mail bij. 

Figuur \ref{fig:Mails} toont hoe de bijlagen worden aangemaakt en hoe de mail verstuurd wordt.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Mails.png}
    \caption[Stappen om een mail te versturen]{Deze afbeelding toont de stappen die nodig zijn in een flow om een automatische mail met bijlagen te versturen}
    \label{fig:Mails}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption[Voorbeeld output van logging]{Deze afbeelding toont de aangepaste rij nadat de gebruiker is aangemaakt. Kolom ``Processed'' is veranderd van ``No'' naar ``Yes''}
...
Onderstaand fragment toont de parameters die gebruikt worden binnen het Exchange Online-script.
...
Onderstaand fragment toont de parameters die gebruikt worden binnen het Active Directory-script.
...
Naast het uitschakelen van het account wordt eveneens de beschrijving van de gebruiker aangepast. Hierbij wordt automatisch geregistreerd sinds wanneer de medewerker uit dienst is en wat de reden van uitdiensttreding was.
...
\begin{figure}[htbp]
    ...
    \caption[Desktopflow offboarding]{Deze afbeelding toont de desktopflow voor het offboardingsproces}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{CloudOffboarding.png}
    \caption[Cloudflow offboarding]{Deze afbeelding toont de cloudflow voor het offboardingsproces}
