...
Binnen het oorspronkelijke onboardingproces van het lokaal gemeentebestuur van Berlare verliepen vrijwel alle stappen volledig manueel. Zowel het aanmaken van gebruikersaccounts als het toekennen van rechten, groepslidmaatschappen en licenties moesten handmatig uitgevoerd worden door medewerkers van de ICT-dienst.

Deze werkwijze was tijdrovend en zorgde ervoor dat medewerkers van de ICT-dienst een aanzienlijk deel van hun tijd moesten besteden aan repetitieve administratieve taken. Hierdoor bleef minder tijd beschikbaar voor complexere technische problemen en andere ondersteunende opdrachten binnen de organisatie. Ook werd de onboarding vaak niet doorgegeven op voorhand. Daardoor moest er meteen geschakeld worden om het in orde te brengen en moesten de andere taken aan de kant geschoven worden. 

Door het onboardingproces te automatiseren via Microsoft Power Automate en PowerShell wordt een groot deel van deze manuele tussenkomsten geëlimineerd. De personeelsdienst kan nu zelfstandig een onboardingaanvraag starten via Microsoft Forms, waarna de verdere verwerking grotendeels automatisch verloopt.

Binnen het oorspronkelijke proces werden gebruikersaccounts eerst handmatig aangemaakt binnen Active Directory. Vervolgens moesten gebruikers afzonderlijk toegevoegd worden aan de correcte groepen en Organizational Units. Nadien moest gewacht worden tot de synchronisatie met Microsoft 365 afgerond was voordat bijkomende cloudconfiguraties uitgevoerd konden worden.

Ook het toekennen van licenties gebeurde manueel. Hoewel de licenties reeds gekoppeld waren aan groepen binnen Microsoft 365, moesten gebruikers nog steeds handmatig toegevoegd worden aan deze groepen nadat de synchronisatie voltooid was. Door deze stappen te automatiseren wordt de werkdruk van de ICT-dienst verminderd en verloopt de configuratie consistenter.

Daarnaast gebeurde ook het toekennen van rechten op gedeelde postvakken volledig manueel. Medewerkers van de ICT-dienst moesten telkens de correcte mailboxen opzoeken en vervolgens afzonderlijk rechten zoals \textit{Full Access} en \textit{Send on Behalf} configureren. Wanneer gebruikers toegang nodig hadden tot meerdere mailboxen, nam deze configuratie bijkomende tijd in beslag.

Hetzelfde gold voor distributiegroepen binnen Microsoft 365. Gebruikers moesten manueel opgezocht worden binnen het Microsoft 365 Admin Center waarna de correcte distributiegroepen één voor één toegevoegd moesten worden.
...
Als laatste werd ook de communicatie handmatig gedaan. Interne communicatie is een werkpunt voor de gemeente en daardoor wordt het soms vergeten. Aangezien de communicatie handmatig gebeurde, als ook soms vergeten werd, waren er vaak andere collega's die niet op de hoogte waren van de status. Met de automatische terugkoppeling die in dit project werd geconfigureerd, wordt de communicatie consistenter en betrouwbaarder. 
...
Tot slot moesten rechten op gedeelde postvakken handmatig geconfigureerd worden. Voor ieder postvak moesten afzonderlijk \textit{Full Access}- en \textit{Send on Behalf}-rechten toegekend worden. Afhankelijk van het aantal mailboxen duurde deze configuratie gemiddeld twee tot vijf minuten.
...
Wanneer de synchronisatietijd buiten beschouwing gelaten wordt, duurde het oorspronkelijke onboardingproces gemiddeld ongeveer twintig minuten actieve werktijd voor de ICT-dienst.

Binnen de geautomatiseerde oplossing verlopen deze stappen grotendeels automatisch via Power Automate en PowerShell. De personeelsdienst start het proces via Microsoft Forms, waarna de verdere configuratie automatisch uitgevoerd wordt zonder voortdurende tussenkomst van de ICT-dienst.
...
Door het onboardingproces te automatiseren wordt deze foutmarge aanzienlijk verkleind. Gegevens worden slechts één keer ingevoerd via Microsoft Forms en vervolgens automatisch verwerkt binnen de verschillende systemen. Hierdoor wordt vermeden dat gegevens opnieuw manueel overgenomen moeten worden.

Binnen de oplossing werden bijkomende controles voorzien om foutieve invoer verder te beperken. Zo moet de personeelsdienst expliciet bevestigen dat de ingevoerde gegevens gecontroleerd werden voordat het formulier verzonden kan worden. Daarnaast worden verschillende waarden automatisch samengesteld via vaste logica binnen Power Automate en PowerShell, waardoor de configuratie consistenter verloopt.

Hoewel menselijke fouten nog steeds mogelijk blijven bij het initiële invullen van het formulier, vermindert de automatisering het aantal manuele handelingen aanzienlijk. Hierdoor wordt de kans op configuratiefouten, vergeten rechten of foutieve groepslidmaatschappen sterk beperkt.
...
Ook binnen het offboardingproces zorgt de automatisering voor een duidelijke vermindering van de benodigde verwerkingstijd. In het oorspronkelijke proces moesten verschillende configuraties handmatig uitgevoerd worden binnen meerdere platformen.
...
Afhankelijk van het aantal groepen, mailboxen en cloudconfiguraties kon een volledige offboarding gemiddeld tien tot vijftien minuten actieve verwerkingstijd vereisen. Dit zeker omdat er geen optie bestaat om bij een gebruiker te zien welke toegang ze hebben op alle mailboxen. Er moet dus handmatig elke mailbox gecontroleerd worden om te zien of de gebruiker hier toegang tot had. 
...
Om onverwachte fouten gecontroleerd op te vangen, wordt binnen de PowerShell-scripts gebruikgemaakt van \texttt{try-catch}-constructies. Hierdoor kunnen fouten tijdens het uitvoeren van opdrachten geregistreerd worden zonder dat het volledige proces onmiddellijk stopt. Daarnaast werden op verschillende plaatsen retry-mechanismen voorzien zodat tijdelijke synchronisatieproblemen tussen Active Directory, Entra ID en Exchange Online automatisch opgevangen kunnen worden.
...
Een eerste beperking is dat bepaalde configuraties nog steeds manueel uitgevoerd moeten worden. Zo worden rechten op specifieke netwerkschijven en bepaalde lokale toepassingen momenteel nog niet automatisch toegekend. Daarnaast blijft ook het voorbereiden van hardware, zoals laptops en randapparatuur, een manuele taak binnen de ICT-dienst.
